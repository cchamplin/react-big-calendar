...
exports.scheduledEventSegments = scheduledEventSegments;
...
function endOfRange(dateRange) {
  var unit = arguments.length <= 1 || arguments[1] === undefined ? 'day' : arguments[1];
...
function scheduledEventSegments(event, first, last, _ref2) {
  var startAccessor = _ref2.startAccessor;
  var endAccessor = _ref2.endAccessor;


  var start = _dates2.default.max((0, _accessors.accessor)(event, startAccessor), first);
  var end = (0, _accessors.accessor)(event, endAccessor);
  if (end > last) {
    end = last;
  }

  var padding = start - first;
  if (padding > 0) {
...
function inRange(e, start, end, _ref3) {
  ...
  var eEnd = _dates2.default.ceil((0, _accessors.accessor)(e, endAccessor), 'day');

  var startsBeforeEnd = _dates2.default.lte(eStart, end, 'day');
  var endsAfterStart = _dates2.default.gt(eEnd, start, 'day');
...
function sortEvents(evtA, evtB, _ref4) {
  var startAccessor = _ref4.startAccessor;
  var endAccessor = _ref4.endAccessor;
  var allDayAccessor = _ref4.allDayAccessor;
